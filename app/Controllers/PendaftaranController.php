...
class PendaftaranController extends BaseController
{
    ...
    public function register()
    {
        ...
        if ($pendaftaranModel->insert($data)) {
            ...
            $email->setMessage("Halo $userName,\n\nTerima kasih telah mendaftar pada event dengan ID: $eventId.\n\nSalam,\nAdmin Event");

            if (!$email->send()) {
                log_message('error', 'Gagal mengirim email: ' . $email->printDebugger(['headers']));
            }

            return redirect()->to('/pendaftaran/sukses')->with('message', 'Pendaftaran berhasil! Email konfirmasi telah dikirim.');
        }
    ...
    public function storeBerbayar()
    {
        ...
        $data = [
            'nama_lengkap' => $this->request->getPost('nama_lengkap'),
            'email' => $this->request->getPost('email'),
            'no_hp' => $this->request->getPost('kode_negara') . $this->request->getPost('no_hp'),
            'alamat_lengkap' => $this->request->getPost('alamat_lengkap'),
            'id_event' => $this->request->getPost('id_event'),
